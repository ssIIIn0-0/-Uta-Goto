...
final class LibraryViewModel {
    ...
    func addSong(title: String, artist: String, lrcContent: String, translations: [String], audioFileName: String? = nil, audioURLString: String? = nil, youtubeURLString: String? = nil, thumbnailData: Data? = nil) {
        guard let context = modelContext else { return }

        let song = Song(title: title, artist: artist, audioFileName: audioFileName, audioURLString: audioURLString, youtubeURLString: youtubeURLString, thumbnailData: thumbnailData)
        ...
        if parsed.isEmpty {
            ...
            for (index, lineText) in lines.enumerated() {
                ...
                let lyricLine = LyricLine(
                    lineIndex: index, originalText: cleaned,
                    translationText: translation, song: song
                )
            ...
            for (index, item) in parsed.enumerated() {
                ...
                let lyricLine = LyricLine(
                    lineIndex: index, originalText: item.text,
                    translationText: translation,
                    timestampStart: item.timestamp, timestampEnd: endTime,
                    song: song
                )
                song.lyricLines.append(lyricLine)
            }
    ...
    private func save() {
        guard let context = modelContext else { return }
        do { try context.save() } catch { errorMessage = "저장에 실패했습니다." }
    }
